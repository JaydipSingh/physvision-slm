...
class PhysVisionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Pad to fixed length. Labels are aligned 1:1 with input_ids here;
        # the model performs the causal shift internally. Padding positions
        # are masked with -100 so they don't contribute to the loss.
        n_real = len(token_ids)
        pad_len = self.max_text_len - n_real
...
def main():
    ...
    val_dataset = PhysVisionDataset(args.data, split="val", tokenize_fn=tokenize_fn)
